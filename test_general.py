...
def test(name, method, url, expected_range, **kwargs):
    try:
...
# 1. REGISTER
r = test("Register", "post", f"{BASE}/api/auth/register", [200, 201],
         json={"username":"testuser","email":"test@test.com","password":"Test123!"})
token = safe_json(r).get("access_token","") if r else ""

# 2. LOGIN
r = test("Login", "post", f"{BASE}/api/auth/login", [200, 200],
         json={"email":"test@test.com","password":"Test123!"})
if r:
    token = safe_json(r).get("access_token", token)

# 3. GET /me
test("Get /me", "get", f"{BASE}/api/auth/me", [200, 200],
     headers={"Authorization": f"Bearer {token}"})

# 4. TELEGRAM CHECK
test("Telegram Check", "get", f"{BASE}/api/auth/telegram/check?telegram_id=12345", [200, 200],
     headers={"Authorization": f"Bearer {token}"})

# 5. NoSQLi PROTECTION
test("NoSQLi Block", "post", f"{BASE}/api/auth/login", [400, 422],
     json={"email":{"$gt":""},"password":"test"})

# 6. ADMIN LOGIN
r = test("Admin Login", "post", f"{BASE}/api/admin/login", [200, 200],
         json={"email":"manobrown333011@gmail.com","password":"Celo050612"})
admin_token = safe_json(r).get("access_token","") if r else ""
admin_headers = {"Authorization": f"Bearer {admin_token}"}

# 7. ADMIN STATS
r = test("Admin Stats", "get", f"{BASE}/api/admin/stats", [200, 200], headers=admin_headers)
if r:
    d = safe_json(r)
...
# 8. ADMIN USERS
r = test("Admin Users List", "get", f"{BASE}/api/admin/users", [200, 200], headers=admin_headers)
if r:
    d = safe_json(r)
...
r = test("Telegram Bot Status", "get", f"{BASE}/api/admin/telegram/status", [200, 200], headers=admin_headers)
if r:
    d = safe_json(r)
    results.append(f"   Online: {d.get('online')} | @{d.get('bot_username','N/A')}")

# 10. DDOS ALERTS
r = test("DDoS Alerts", "get", f"{BASE}/api/admin/ddos/alerts", [200, 200], headers=admin_headers)
if r:
    d = safe_json(r)
    results.append(f"   Active: {d.get('active_alerts',0)} | Threshold: {d.get('threshold','?')}")

# 11. VISIT TRACKING
test("Visit Track", "post", f"{BASE}/api/admin/visits/track", [200, 200])

# 12. ALL SCANS
test("Admin All Scans", "get", f"{BASE}/api/admin/scans/all", [200, 200], headers=admin_headers)

# 13. ADMIN BAD CREDS
test("Admin Bad Creds", "post", f"{BASE}/api/admin/login", [401, 401],
     json={"email":"wrong@test.com","password":"wrong"})

# 14. PROTECTED ROUTE WITHOUT TOKEN
test("Protected No Token", "get", f"{BASE}/api/admin/stats", [401, 403])

# 15. LOGOUT
test("Logout", "post", f"{BASE}/api/auth/logout", [200, 200],
     headers={"Authorization": f"Bearer {token}"})

# 16. DELETE TEST USER
test("Delete Test User", "delete", f"{BASE}/api/admin/users/testuser_id", [200, 404], headers=admin_headers)
